...
[`proof/matrix.md`](proof/matrix.md). 887 tests, standard library only,
offline.
...
| Code Quality | Applies (not met). `make lint` runs ruff's default rule set and `mypy --strict` against `src/plumbline`, and both are green; ruff's floor is pinned at `ruff>=0.15.0`, mypy's at `mypy>=1.13.0`. F541 is the one ruff rule ignored, with the reason in `pyproject.toml`: editing a source file moves `harness_source_sha256` and invalidates every committed audit, which is too high a price for a redundant `f` prefix. Branch coverage over `src/` is 94% against a 90% floor, enforced by `make test`. mypy's default-mode gap closed 2026-08-22 (#20) and its `--strict` gap closed 2026-08-23: 174 findings fixed rather than suppressed — no `# type: ignore`, no narrowed scope — across 28 files, mostly a bare `dict` given a real type argument; see `docs/metrics-ledger.md` for the measured trail. Measured 2026-09-13 on `ruff 0.16.5`: two real gaps remain — the wider portfolio ruff rule set has 557 findings (453 of them line length, the same whole-tree reformat question `ruff format --check` above already names), and 23 functions exceed a McCabe complexity of 10. This sentence previously read 352 (264) and 18 on `ruff 0.16.3`, and 326 (241) and 17 before that; the ledger carries each measurement with the tool version it was taken on, because a ruff upgrade moves these counts on code that did not change and a bare number cannot say which of the two happened. That is exactly what happened here: not one line of `src/plumbline/` changed between the last two readings, and re-measuring at the base commit of the branch that updated this sentence gives the same 557 and 23, so the whole movement is the lockfile bump. Python floor is 3.11, below the portfolio's 3.12 |
...
| Observability | Applies (Tier C). A command-line harness writing reports to a directory. No service, no telemetry, no SLO surface. The observable surface is the exit-code contract and the report, both tested. [`docs/operations-runbook.md`](docs/operations-runbook.md) closes the gap this row named against itself: what actually breaks (keyed to each exit code, plus the CI failure modes this repository has hit), how you'd notice, and what to do — split for the two different people who hit it, someone gating a repository with Plumbline and someone maintaining this one |
