...
- **Two refusal rates were reported as `0.0` when nothing had been measured,
  ...
  over-refusal rate (refused what it should have answered) and an
  under-refusal rate (answered what it should have refused). Each has its own
  denominator — the items that could have gone wrong in that direction — and
  either denominator is zero in a perfectly ordinary bundle: a safety-probe
  set is all `refuse`, a guidance-accuracy set is all `answer`. Both rates
  read `len(failures) / n if n else 0.0`. So a bundle in which the target was
  never once asked to answer published `over_refusal_rate: 0.0` into
  `report.json`, which says "asked many times, never wrongly refused". The
  opposite of what happened, in the field a consumer reads programmatically.

  A rate with no denominator is now `null`, and the two denominators
  (`n_should_answer`, `n_should_refuse`) are reported beside the rates so a
  null is readable and a zero is checkable — a reader could not previously
  tell a rate measured over 170 items from one measured over two. A measured
  zero is still `0.0`; that distinction is the point, and `details.rate_note`
  states it in the report itself. Three tests pin it, each observed failing on
  the previous `refusal.py` with `AssertionError: 0.0 is not None`.

- **`accuracy` was the one suite that narrowed its own population without
  refusing an empty one.** It scores the items whose expected behavior is
  `answer`, a subset that can be empty while the bundle is not. Every other
  suite that filters — `adversarial`, `fairness`, `conduct`, `grounding`,
  `attribution`, `cross_language`, `conversational_integrity`,
  `accessibility` — calls `require_population` and fails closed. `accuracy`
  had `pooled = sum(scores) / n if n else 0.0` instead, so an all-`refuse`
  bundle scored `0.0` over `n = 0` and failed: fail-closed in the verdict, but
  a number nothing measured, and one that reads in the report as "the target
  answered everything wrong" rather than "this bundle holds no answer to
  score". It now raises `EmptyPopulationError` like its siblings, which the
  CLI already renders as a configuration error (exit 4). The remaining
  `if n else 0.0` fallbacks in `refusal`, `smoke` and `multilingual` score
  `bundle.items` whole, and `bundle.py` refuses a bundle with no items, so
  their denominator cannot be zero.

  Demo artifacts regenerated by the runbook's documented order:
  `audits/c3935444f4e3b779` replaces `a513584ab2953e27`, baseline, matrix and
  site rebuilt from it, second independent run byte-identical. No demo score
  moved and no verdict changed — the riverbend bundle has both directions and
  answer items, so neither fix reaches it; what moved is the harness source
  hash and the two new denominators in `refusal`'s details.
