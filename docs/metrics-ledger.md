...
| 2026-08-29 | published-claims gate (#44) | 15 | 651 | 94% | 0 | 352 (264) | 18 | 0 (0) | Measured on `ruff 0.16.3` and `mypy 2.3.1`, which is the first row here to record the tool versions, and it is why it exists. The README's code-quality row carried figures dated 2026-08-23 — 326 wide findings, 241 of them line length, 17 over complexity 10 — that this ledger had no row for. Re-running the same commands today gives 352 (264) and 18. Some of that is this repository and some of it is a newer ruff, and nothing recorded at the time can separate the two, so the 2026-08-23 figures are not back-filled here: this row is a fresh measurement on a stated tool version, and the README now points at it instead. `--strict` mypy is 0 as well as default mypy, closing the `0 (174)` gap the three rows above carried. Tests stood at 644 on `2bd2626`, the commit this branch starts from, and at 651 here: `tests/test_claims.py` adds seven. |
| 2026-09-13 | ideas-page status gate (branch `claude/features-expansion-agents-nd4aec`) | 15 | 887 | 94% | 0 | 557 (453) | 23 | 0 (0) | Measured on `ruff 0.16.5` and `mypy 2.3.1`. Documentation and tests only: no `src/plumbline` file changed, so suites, coverage and both mypy columns are unmoved. Tests 866 → 887; `tests/test_expansion_status.py` adds 21. The wide-ruff and complexity columns moved 352 (264) → 557 (453) and 18 → 23 on code that did not change: re-measuring at this branch's base commit (`aec1fcc`) gives the same 557 (453) and 23, so the whole movement is the `ruff 0.16.3` → `0.16.5` bump in `uv.lock` and none of it is this change. This is the second time that has happened and it is the reason the 2026-08-29 row exists. `tools/check_expansion_status.py` adds nothing under either set — its `check()` measured 13 on McCabe in the first draft and was split into three functions, rather than widening a recorded gap by one and recording that instead. |
